...
* FIX: read NEXRAD Level 2 LDM control word as signed int32 — the last compressed record's size is negative by convention, and the unsigned read caused a ~4 GiB allocation / ``MemoryError`` on most archive volumes by [@swnesbitt](https://github.com/swnesbitt)
